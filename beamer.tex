...
\usetheme[font=arial, elevatedtitles=false]{NMBU}

% use elevatedtitles=true to move the titles up
\usefonttheme[onlymath]{serif} % Remove this if you want arial math

\usepackage{lmodern}

% Arial font with XeLaTeX or LuaLaTeX
\usepackage{fontspec}
\setsansfont{Arial}
\setmainfont{Arial}
...
%========================
\section*{Overview}
\begin{frame}{How to use this file}
\begin{itemize}
  ...
  \item Compile with \textbf{XeLaTeX} or \textbf{LuaLaTeX} to use the theme's font settings.
...
This slide demonstrates a simple text layout. Paragraphs are set for readability on large screens. Use concise sentences and limit the number of bullets per slide to keep attention on key points.

Inline math like $a^2+b^2=c^2$ and short expressions $x \in \mathbb{R}$ render clearly. Hyperlinks are legible yet unobtrusive: \url{https://example.org}.
\end{frame}
